...
namespace Testing.Controller
{
    public class DepartmentControllerTest
    {
        ...
        [Fact]
        public void UpdateDepartment_ShouldReturnStatusCode200_WhenDepartmentIsValid()
        {
            Department department=DepartmentMock.GetvalidDepartment();
            _departmentService.Setup(obj=>obj.UpdateDepartment(department)).Returns(true);
            var Result=_departmentController.Update(department) as ObjectResult;
            Assert.Equal(200,Result?.StatusCode);
        }
        [Fact]
        public void UpdateDepartment_ShouldReturnStatusCode400_whenDepartmentThrowsValidationException()
        {
            Department department=DepartmentMock.GetvalidDepartment();
            _departmentService.Setup(obj=>obj.UpdateDepartment(department)).Throws<ValidationException>();
            var Result=_departmentController.Update(department) as ObjectResult;
            Assert.Equal(400,Result?.StatusCode);
        }
        [Fact]
        public void UpdateDepartment_ShouldReturnStatusCode400_whenDepartmentThrowsException()
        {
            Department department=DepartmentMock.GetvalidDepartment();
            _departmentService.Setup(obj=>obj.UpdateDepartment(department)).Throws<Exception>();
            var Result=_departmentController.Update(department) as ObjectResult;
            Assert.Equal(400,Result?.StatusCode);
        }
    }
